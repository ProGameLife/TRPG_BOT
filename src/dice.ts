...
export const set_dice = async (message: Message<boolean>) => {
    ...
    const result_dice = await roll_dice(Number(times), Number(dice_number), Number(extra_number), result_dice_number, 0);
    await message.channel.send( message.content + ' 주사위 결과 : ' + result_dice.toString())
};

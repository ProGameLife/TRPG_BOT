...
export const COMMAND_GUIDE =
    R_L + ' ** TRPG 서버 명령어 모음 ** ' + R_R + NX_LINE +
    R_S + ' ``!특성치`` 입력 시 특성치에 관한 명령어 확인이 가능합니다.' + NX_LINE +
    R_S + ' ``!특성치 자동`` 입력 시 특성치를 자동으로 생성 가능합니다.' + NX_LINE + 
    R_S + ' ``!특성치 수동`` 입력 시 특성치를 수동으로 입력 가능한 가이드가 표시 됩니다.' + NX_LINE + 
    R_S + ' ``!특성치 수동``을 입력 하신 경우 ``!근력 XX`` 명령어를 사용 가능합니다. 입력 시 해당 특성을 정합니다.' + NX_LINE + 
    R_S + ' ``!특성치 초기화`` 입력 시 특성치를 초기화합니다.' + NX_LINE + 
    R_S + ' ``!특성치 입력 완료`` 특성치를 정하고 **필수적**으로 입력해야 하는 명령어 입니다.' + NX_LINE + 
    R_S + ' ``!기능`` 입력 시 기능에 관한 명령어 확인이 가능합니다.' + NX_LINE + 
    R_S + ' ``!기능목록`` 입력 시 현재 사용가능한 기능의 목록이 확인 가능합니다.' + NX_LINE + 
    R_S + ' ``!기능추가 관찰력 XX`` 입력 시 관찰력에 대한 기능의 값이 XX만큼 증가합니다.' + NX_LINE + 
    R_S + ' ``!기능초기화`` 입력 시 현재까지 입력한 기능의 값이 초기화 됩니다.' + NX_LINE + 
    R_S + ' ``!정보`` 입력 시 PMC 설정에 관한 명령어 확인이 가능합니다.' + NX_LINE + 
    R_S + ' ``!이름설정 XXX`` 입력 시 PMC 이름설정이 가능합니다.' + NX_LINE + 
    R_S + ' ``!직업설정 XXX`` 입력 시 PMC 직업설정이 가능합니다.' + NX_LINE + 
    R_S + ' ``!나이설정 XXX`` 입력 시 PMC 나이설정이 가능합니다.' + NX_LINE + 
    R_S + ' ``!성별설정 XXX`` 입력 시 PMC 성별설정이 가능합니다.' + NX_LINE + 
    R_S + ' ``!사진설정 XXX`` 입력 시 PMC 이미지 설정이 가능합니다.' + NX_LINE + 
    R_S + ' ``!백스토리`` 입력 시 PMC 백스토리 설정에 관한 명령어 확인이 가능합니다.' + NX_LINE + 
    R_S + ' ``!백스토리입력 XXX~~~XXX`` 입력 시 PMC 백스토리 설정이 가능합니다.' + NX_LINE + 
    R_S + ' ``!장비`` 입력 시 PMC 장비 설정에 관한 명령어 확인이 가능합니다.' + NX_LINE +
    R_S + ' ``!장비추가 XXX`` 입력 시 PMC가 시나리오에 들고갈 장비를 추가합니다.' + NX_LINE + 'ㅤ';

export const COMMAND_GUIDE2 = 
    NX_LINE + Y_L + ' !KPC로 시작하는 명령어는 KPC에게 문의 하고 사용 하시기 바랍니다.' + Y_L + NX_LINE +
    Y_S + ' ``!KPC 디코프로필ID 변경항목 변경값`` 변경 항목에는 모든 특성치가 해당 됩니다.' + NX_LINE +
    Y_S + ' ``!KPC 디코프로필ID 빈사(광기) 빈사상태(광기상태)`` 광기와 빈사 상태를 수정 할 수 있습니다.' + NX_LINE +
    Y_S + ' ``!KPC 디코프로필ID 정보삭제`` 해당 사용자의 탐사자 정보를 삭제합니다.' + NX_LINE +
    Y_S + ' ``!KPC 디코프로필ID 스킬포인트 100`` 해당 탐사자의 스킬포인트를 수정합니다.' + NX_LINE +
    Y_S + ' ``!템플릿 저장 탐사자ID`` 해당 탐사자의 정보를 템플릿으로 저장합니다.' + NX_LINE +
    Y_S + ' ``!템플릿 목록`` 현재 템플릿의 목록을 확인 합니다.' + NX_LINE +
    Y_S + ' ``!템플릿 확인 템플릿번호`` 해당 템플릿번호에 해당하는 템플릿의 특성과 기능, 백스토리 등등 확인 가능합니다.' + NX_LINE +
    Y_S + ' ``!템플릿 불러오기 템플릿번호`` 템플릿번호에 해당하는 정보를 현재 탐사자에게 덮어씁니다.' + NX_LINE;
...
export const MAKE_ABILITY_GUIDE =
    ...
    R_S + ' ``!특성치 입력 완료`` **!!필수적!!**으로 해당 명령어를 입력하여 완료 해주세요.' + NX_LINE;
...
export const KPC_GUIDE =
    ...
    G_S + ' ``!템플릿 확인 템플릿번호``' + NX_LINE +
    G_S + ' 예시 ) !템플릿 확인 4' + NX_LINE;